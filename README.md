# 謎解きパーティー

スマホで2人以上で楽しむ、ナゾトキ協力プレイアプリ。

**公開URL**: https://anzaihikaru.github.io/nazotoki-party/

![QR](qr.png)

## 機能

- **ストーリーモード**: 難易度別に3本収録
  - イージー：「消えた指輪事件」（全5話）
  - ノーマル：「お母さんのたんじょう日 宝さがし」（全4話）
  - ハード：「消えた数学者の遺書」（全5話）
- **ひたすら謎解きモード**: 各難易度のプールからランダム出題、無限に続く
- **おひとり様モード**: 1人で気軽に挑戦（ターン交代・スコア対決なし）
- **難易度**: イージー / ノーマル / ハード（ひたすらモードに合計85問を収録）
- **2〜6人プレイ**: 同じスマホを回しながら順番に回答
- **スコア**: 正解 +10pt、ヒント使用後の正解 +5pt
- **答えを表示**: 行き詰まったときの救済（その問題は0点）
- **更新お知らせ**: バージョンアップ後の初回起動時にだけ通知
- **メモ機能**: 画面右下📝ボタンで全画面メモ。3色ペン / 消しゴム / 全消し
- **画像系の謎**: SVG・絵文字・影絵・鏡像時計・展開図・錯視など視覚パズル多数

## ローカルで動かす

`index.html` をブラウザでダブルクリックで開くだけ。
ビルドや依存パッケージは不要（Vanilla HTML/CSS/JS）。

スマホで遊ぶときは（同じWi-Fi内）：

```powershell
cd C:\Users\hikar\OneDrive\デスクトップ\謎解き
npx http-server -p 8080 -a 0.0.0.0
# → http://<PCのIP>:8080/  をスマホで開く
```

## ファイル構成

```
index.html   画面構造
style.css    スタイル（モバイルファースト）
puzzles.js   謎データ + APP_VERSION
memo.js      Canvas手書きメモ
app.js       画面遷移とゲーム進行
qr.png       本番URLのQRコード
```

## 謎を追加する

`puzzles.js` の `ENDLESS[難易度]` 配列に追加するだけ。

```js
{
  type: "なぞなぞ",           // 表示カテゴリ（絵文字つけても可）
  q: "問題文",
  html: `<div>SVG等のHTML</div>`, // ← 任意。画像系の謎で使用
  hint: "ヒント文",
  a: ["正解", "代替正解"],
  // choices: ["選択肢1", "選択肢2"],  // ← あれば選択式に
}
```

## デプロイ

GitHub Pages で公開中。ソース更新 → push で自動反映：

```powershell
git add -A
git commit -m "新しい謎を追加"
git push
```

1〜2分後に https://anzaihikaru.github.io/nazotoki-party/ が更新されます。

## バージョン履歴

- **v0.5.0** (2026-06-13)
  - 新作の謎を30問追加（各難易度に10問ずつ、合計85問に）
    - イージー：イラスト頭文字・隠し言葉・逆さ読み・影絵・図形数え など
    - ノーマル：ありなし・曜日暗号・反対語暗号・マッチ棒・ひっかけ など
    - ハード：正直村うそつき村・トリボナッチ数列・ローマ字逆読み暗号・時計算・カレンダー算数 など

- **v0.4.0** (2026-05-21)
  - ストーリーモードを難易度別に分岐
    - イージー：「消えた指輪事件」（従来通り・全5話）
    - ノーマル：「お母さんのたんじょう日 宝さがし」（新規・全4話）
    - ハード：「消えた数学者の遺書」（新規・全5話）
  - 新しい謎表示用のCSS（ペアリスト・好き嫌い・キーパッド表）を追加

- **v0.3.0** (2026-05-20)
  - 視覚パズル30問を追加（視点変更・図形数え・鏡時計・折り紙・展開図・錯視など）
  - 「📖 答えを表示」ボタン追加（行き詰まり救済、その問題はスコア対象外）
  - 「👤 おひとり様モード」追加（1人プレイ：ターンなし／対決なし）
  - 更新お知らせモーダルを追加（端末ごと初回起動時のみ表示、localStorageで記憶）
  - 旧版にあった「単純暗号」「頭文字」「アナグラム」系の謎の一部を整理

- **v0.2.0** (2026-05-20)
  - 謎を全面リニューアル（合計33問+ストーリー5話）
  - 大人なぞなぞ系（同音異義・ダジャレ）を多数追加
  - 画像ベース謎を追加：emoji・ありなし・グリッド・SVG暗号
  - タイトル/結果画面にバージョン表示
- **v0.1.0** (2026-05-20) - 初版
  - 基本フレーム、ストーリー＋ひたすら2モード
  - メモ機能、スコア機能、難易度3段階
